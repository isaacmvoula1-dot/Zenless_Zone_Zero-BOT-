...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '242068450932@s.whatsapp.net'; // ton numéro ou le groupe cible
                const imagePath = './database/ZZZCo.jpg';
                ...
                const messageText = `
╔══════════════════╗
      *Zenless_Zone_Zero-BOT Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. Zenless Zone Zero, one of the best."
╚══════════════════╝
...
*Zenless Zone Zero*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by Zenless Zone Zero',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 242068450932; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '242068450932' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'ZenlessZoneZero');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['242068450932@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
